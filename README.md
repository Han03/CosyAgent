...
├── config/          # AgentProperties、ChatClient 配置
├── agent/
│   ├── core/        # AgentState / AgentMessage / AgentContext / ReActAgent 契约
│   ├── tool/        # AgentTool 契约 + ToolRegistry + 示例工具
...
| Step 2 | ReAct 编排：LLM 接入、工具桥接（@Tool）、Thought→Action→Observation 循环 | 待实施 |
...
```bash
mvn test   # 上下文加载 + 工具注册表单测 + Agent HTTP 接口集成测试
```
